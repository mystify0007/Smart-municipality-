...
function normalizeStatus(status) {
  const map = { Pending: "Pending", Approved: "Completed", Rejected: "Rejected" };
      ...
      <div className="bg-portal-panel border border-portal-panel-border rounded-xl p-6">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {requests.map((r) => {
                const display = normalizeStatus(r.status);
                return (
                  <tr key={r.certificate_id} className="border-b border-portal-panel-border/50">
                    ...
                    <td className="py-3 text-portal-muted">{r.purpose}</td>
                    <td className="py-3 text-portal-muted">{new Date(r.applied_date).toLocaleDateString()}</td>
                    <td className="py-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs ${STATUS_STYLES[display] || ""}`}>
                        {display}
                      </span>
                    </td>
                    <td className="py-3 text-portal-muted">{r.remarks || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
